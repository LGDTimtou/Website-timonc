...
const projects = {
    ...
    gaussianSplatting: {
        ...
        short_description:
            "My Master's thesis, on the initialization step of 3D Gaussian Splatting — a technique that represents a scene as a large set of small 3D Gaussians and renders it in real time. Before optimization those Gaussians have to be placed somewhere; the standard approach seeds them from a sparse point cloud, which is thin in low-texture areas and says little about where the scene's detail actually sits. I build a denser, structured starting point: each frame of colour and depth is split along photometric and geometric edges into a tree of regions — small where the scene is complex, large where it is flat — and one Gaussian is placed per region using LiDAR-completed depth. The optimization then converges faster and reaches comparable or better quality with fewer Gaussians.",
        description:
